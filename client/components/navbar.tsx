...
export default function Navbar() {
  ...
  const toggleHidden = () => {
    // console.log('toggling hanburger');
    setActive(!active);
  };

  const hideAndScrollTo = (url: string) => {
    toggleHidden();
    smoothScrollToSection(url);
  };

  return (
    <div className="fixed top-0 z-50 bg-gray-950 w-full m-0">
      <div className=" shadow">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between py-4">
            <div>
              <div className="w-10 h-10"></div>
              {/* <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 text-cyan-400" viewBox="0 0 24 24">
            ...
            <div className="hidden md:flex md:items-center">
              {menu.map((data: any, index: number) => (
                <Link
                  className="text-white text-sm font-semibold hover:text-cyan-400 mr-4"
                  href="#"
                  scroll={false}
                  onClick={() => smoothScrollToSection(data.url)}
                  key={index}>
                  {data.name}
                </Link>
              ))}
            ...
            <div className="md:hidden cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-cyan-400" viewBox="0 0 24 24" onClick={toggleHidden}>
          ...
          <div className={`${active ? '' : 'hidden'} bg-gray-950 border-t-2 py-2`}>
            <div className="flex flex-col">
              {menu.map((data: any, index: number) => (
                <Link
                  className="text-white text-sm font-semibold hover:text-cyan-400 mb-1"
                  href="#"
                  scroll={false}
                  onClick={() => hideAndScrollTo(data.url)}
                  key={index}>
                  {data.name}
                </Link>
              ))}
